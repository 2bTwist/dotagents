...
- **Read files fully** (no `limit`/`offset`). You need complete context.
- Think deeply about how the pieces fit together
- Create a todo list (via TaskCreate) to track your progress
...
- Run the success criteria checks (typically `pnpm lint && pnpm typecheck && pnpm test` or equivalent — the plan should specify exact commands)
...
If instructed to execute multiple phases consecutively, skip the pause until the last phase. Otherwise, assume you are doing one phase.
