...
- **ASO + keyword-ranks scoped to the BeSeen project.** `aso-audit`, `aso-and-apple-ads`, and `keyword-ranks` are app-specific; they now live in `~/Projects/BeSeen/.claude/skills/` (local/gitignored per BeSeen convention) and were deleted from all global dirs (`~/.claude`, `~/.agents`, `~/.pi/agent`). Removed the stale `domain-skills/claude/{aso-audit,aso-and-apple-ads.md,keyword-ranks}` and `domain-skills/pi/aso-audit` copies from this repo so `install.sh --with-domain-skills` can't re-add them globally.
